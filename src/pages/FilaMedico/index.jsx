...
import { GetfilaMedico, PutFila } from "../../service/Fila";
import Toastify from "../../components/Toastify";
import { InserirConsulta } from "../../service/consulta";

export default function FilaMedico() {
  ...
  useEffect(() => {
    const getPacientes = async () => {
      try {
        const data = await GetfilaMedico(page);
  ...
  const handlePacienteClick = async (paciente, event) => {
    ...
    if (paciente.status === "aguardando" || paciente.status === "Aguardando") {
      ...
      if (result.sucess) {
        ...
        gerarConsulta(paciente);
      } else {
            if (result.data && result.data.erros && result.data.erros.length > 0) {
              toast.error(result.data.erros[0]);
            } else {
              toast.error("Erro ao encaminhar para consulta.");
            }
          }
    }
  };

  const gerarConsulta = async (paciente) => {
    const token = localStorage.getItem("token");
    const consulta = {
      dataDaConsulta: formatDate(),
      id_colaborador: 3,
      id_fila: paciente.id,
      diagnostico: "null",
    };

    const result = await InserirConsulta(consulta, token);
    if (result.sucess) {
              ...
              <Table>
                <thead>
                  <TableRow>
                    ...
                    <TableHeader tabIndex={0}>Especialidade</TableHeader>
                  </TableRow>
                </thead>
                <tbody>
                  {fila.map((item, index) => (
                    <TableRow key={index} tabIndex={0}>
                      ...
                      <TableCell tabIndex={0}>{item.especialidade}</TableCell>
                    </TableRow>
                  ))}
                </tbody>
              </Table>
